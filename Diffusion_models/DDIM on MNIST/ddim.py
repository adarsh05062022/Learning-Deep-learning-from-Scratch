...
LR = 2e-4
SAVE_DIR = "ddim_mnist_checkpoints"
...
transform = transforms.Compose([
    transforms.Resize(32),               # <- CHANGE to 32
    transforms.ToTensor(),               # 0..1
    transforms.Lambda(lambda x: x * 2.0 - 1.0)  # to [-1,1]
])
...
def train(model, optimizer, epochs=EPOCHS):
    ...
    for ep in range(epochs):
        ...
        torch.save(model.state_dict(), os.path.join(SAVE_DIR, f"ckpt.pt"))
        ...
        save_images(samples, nrow=8, filename=f"epoch_{ep}_50.png", save_dir="results")

        # Also try faster (very few steps)
        print("Sampling with DDIM (20 steps)...")
        samples2 = ddim_sample(model, batch_size=64, num_steps=20, eta=0.0)
        save_images(samples, nrow=8, filename=f"epoch_{ep}_20.png", save_dir="results")
    return model
