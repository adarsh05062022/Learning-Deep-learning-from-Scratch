...
def save_generated_images(generator,labels, epoch, device,  image_dir,num_images=16):
    os.makedirs(image_dir, exist_ok=True)  # ensure folder exists

    # Match labels to z
    sample_labels = labels[:num_images].to(device)

    # sample noise
    z = torch.randn(num_images, 100,1,1).to(device)
    generated_imgs = generator(z,sample_labels).detach().cpu()
