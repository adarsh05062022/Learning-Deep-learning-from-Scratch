    ...
    for idx, (zebra, horse) in enumerate(loader):
        ...
        if idx % 100 == 0:
            print(
                ...
                f"Step [{idx}/{len(loader)}] "
                f"H_real={H_reals/(idx+1):.3f}, H_fake={H_fakes/(idx+1):.3f} "
...
def main():
    ...
    for epoch in range(config.NUM_EPOCHS):
        ...
        # ---------------------
        # Save validation samples per epoch
        # ---------------------
        ...
        for i, (zebra, horse) in enumerate(val_loader):
            if i == rand_idx:
                ...
                with torch.no_grad():
                    fake_horse = gen_H(zebra)
                    fake_zebra = gen_Z(horse)
                save_image(fake_horse * 0.5 + 0.5, f"saved_images/horse_epoch_{epoch+23}.png")
                save_image(fake_zebra * 0.5 + 0.5, f"saved_images/zebra_epoch_{epoch+23}.png")
                break
